...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'popshock': // Popshock
				...
				var stageFront:BGSprite = new BGSprite('Background2', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					schoolIntro(doof);
				case 'rebooted':
					snapCamFollowToPos(400, 400);
					startVideo('Cutscene1');
				case 'neon-nits':
					startVideo('Cutscene2');
				case 'kernal-panic':
					startVideo('pspplusportal1_4');
				case 'battlefield-frenzy':
					startVideo('twcutscenecomp');
				case 'nullruption':
					snapCamFollowToPos(400, 400);
					startVideo('jamcutscenecomp');
				case 'rescooted':
					startVideo('bfpostalcutscenecomp');
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['neon-nits','rescooted', 'nullruption', 'kernal-panic', 'battlefield-frenzy', 'friday_night_play']);
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'debugger':
						...
						case 'neon-nits':
						if (Paths.formatToSongPath(SONG.song) == 'neon-nits' && !usedPractice ) {
							unlock = true;
						}
					case 'rescooted':
						if (Paths.formatToSongPath(SONG.song) == 'rescooted' && !usedPractice ) {
							unlock = true;
						}
					case 'nullruption':
						if (Paths.formatToSongPath(SONG.song) == 'nullruption' && !usedPractice ) {
							unlock = true;
						}
					case 'kernal-panic':
						if (Paths.formatToSongPath(SONG.song) == 'kernal-panic' && !usedPractice ) {
							unlock = true;
						}
					case 'battlefield-frenzy':
						if (Paths.formatToSongPath(SONG.song) == 'battlefield-frenzy' && !usedPractice ) {
							unlock = true;
						}
				}
